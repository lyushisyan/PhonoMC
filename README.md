...
NTMC 是一个基于 C++ 开发的、利用声子蒙特卡洛（Phonon Monte Carlo）方法模拟半导体材料（如 Si, Ge, GaN）中声子输运过程的仿真框架。该工具支持复杂几何结构，并能通过 HDF5 文件加载真实的声子色散和散射数据，用于预测微纳尺度的热流和温度分布和等效热导率。
...
* **统计输出**：实时计算温度梯度、热流以及基于线性拟合和两端温差的有效热导率。
...
模拟开始后，结果将保存在 results/ 目录下以项目名命名的文件夹中：
  ...
  `convergence.txt`: 核心数据文件，包含步数、各网格温度（`T_1 ... T_n`）、实时热流及热导率预测值。
